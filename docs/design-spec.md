...
| Deviation | Why |
|---|---|
| **Diagnostics** — the `MIC`/`UNPROCESSED` toggle and the frame counters, **hidden until the wordmark is long-pressed** | Not in the design, which correctly excludes settings screens. But [S2 and S3](mvp-scope.md) exist to answer [H4](feasibility.md) and to run [Scenario G](test-matrix.md), and the A/B cannot be driven from `adb`. It first shipped as a collapsed row on screen and that was wrong: the height it took pushed the listening state text into a scrolling box |
| **An overflow menu on the wordmark row** → privacy policy, terms of use, about ([M16](mvp-scope.md)) | Play requires the privacy policy to be reachable inside the app, and a product left in a child's room needs its "this is not a safety device" statement somewhere. The wordmark row already existed, so the documents cost no height on the screen that matters. Hidden while dimmed, with everything else that is not state or Stop |
| **"Listening stopped on its own" notice** ([S5](mvp-scope.md)) | The design has no slot for it. Rendered as a `paused`-coloured row in the status panel — it is a report about a past session, not a current state, and must not be confusable with one |
| Headphone row shows the device's own name, falling back to **"Bluetooth headphones"** | The design mock said "Buds Pro" and the first build showed `BLUETOOTH_A2DP` — a debug string in user copy. `AudioDeviceInfo.getProductName()` is used rather than `BluetoothDevice.getName()`, which would need `BLUETOOTH_CONNECT` that [ADR-0007](adr/0007-minimal-permission-set.md) excludes. The notification still says *your headphones*, since it has no room to be conditional |
