...
- **`AudioManager.getDevices()` exposes device *types* without any Bluetooth permission.** Only the
  human-readable product *name* requires `BLUETOOTH_CONNECT`. The UI needs a status dot, not a name.
...
| `BLUETOOTH_CONNECT` | Only needed for device *names*; types suffice |
...
- Bluetooth device *names* cannot be shown. Accepted — brief §9's UI needs a dot, not a label.
...
**Add `BLUETOOTH_CONNECT` to show device names.** Nicer status text for a runtime permission
prompt on a privacy-sensitive app. Rejected; the dot is enough.
